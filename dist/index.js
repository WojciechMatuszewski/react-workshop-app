...
function makeKCDWorkshopApp({
  ...
  function handleLocationChange(location = history.location) {
    ...
    document.body.classList.remove(previousLocation.pathname.replace(/\//g, '_'));
    document.body.classList.add(pathname.replace(/\//g, '_')); // set the title to have info for the exercise
    ...
    isolatedModuleImport().then(async ({
      ...
      if (typeof defaultExport === 'function') {
        ...
        for (const script of scripts) {
          ...
          if (script.hasAttribute('src')) {
            loadingScriptsQueue.push(new Promise(resolve => newScript.onload = resolve));
          }
...
var _default = makeKCDWorkshopApp;
/*
eslint
